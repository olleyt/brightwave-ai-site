...
2. **Exercise judgment on structure.** Decide whether the source is one topic or several — a long doc covering three distinct services should become three topics, not one bloated one. For each topic, draft:
   ...
   If the source has a literal URL — you fetched it via `WebFetch`/`curl`, or it's a Notion page with a shareable link — add a `source` field on the topic that **includes that exact URL**, not just a description of it: `"source": "AWS 'Logical Separation' whitepaper — https://docs.aws.amazon.com/pdfs/whitepapers/latest/logical-separation/logical-separation.pdf"`. A name alone ("AWS whitepaper") isn't a citation — the point is a link someone can actually follow back to the original. For a local file with no public URL, cite the filename instead (or skip `source` if there's nothing traceable to point to). This field isn't part of the validated schema, just a plain passthrough string that rides along in the topic object — costs nothing and keeps a paper trail back to where the facts came from, which matters since these cards paraphrase third-party material.
